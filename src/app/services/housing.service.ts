...
export class HousingService {
  ...
  getHousingLocationById(id: number): HousingLocation | undefined {
    const res = this.housingLocationList.find((item) => item.id === id);
    return res;
  }

  searchHousingLocations(name: string): HousingLocation[] {
    if (!name) return this.housingLocationList;
    const lst = this.housingLocationList.filter((item) => {
      return item.name
        .toString()
        .toLowerCase()
        .trim()
